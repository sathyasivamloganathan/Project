...
def create_pdf_from_dictionary_and_store_to_admin_saves(data, grid_fs_adminSaves, sender_email, hospital_Name, doctor_name, doctor_email, receiver_email, receiver_name, patient_id, patient_mobile):
    try:
        ...
        pdf_buffer.seek(0)
        metadata = {'SenderEmail': sender_email,'HospitalName': hospital_Name, 'DoctorName': doctor_name, 'DoctorEmail': doctor_email, 'ReceiverEmail': receiver_email, 'ReceiverName': receiver_name, 'PatientId': patient_id, 'PatientMobile': patient_mobile}
...
def adminSavedData():
    if request.method == 'POST':
        try:
            ...
            create_pdf_from_dictionary_and_store_to_admin_saves(data, grid_fs_adminSaves, sender_email, hospital_Name, doctor_name, doctor_email, receiver_email, receiver_name, patient_id, patient_mobile)
...
def create_pdf_from_dictionary_and_store_to_adminUser_saves(data, grid_fs_adminSends, sender_email, hospital_Name, doctor_name, doctor_email, receiver_email, receiver_name, patient_id, patient_mobile):
    try:
        output_filename = f"{patient_id}.pdf"
        pdf_buffer = io.BytesIO()
        pdf_canvas = canvas.Canvas(pdf_buffer, pagesize=(612, 792))
...
def adminSendToUserData():
    if request.method == 'POST':
        try:
            ...
            create_pdf_from_dictionary_and_store_to_adminUser_saves(data, grid_fs_adminSends, sender_email, hospital_Name, doctor_name, doctor_email, receiver_email, receiver_name, patient_id, patient_mobile)
...
def check_user_details():
    ...
    user = mongo.db.userData.find_one({"email": receiver_email})
    if user:
        return jsonify({"found": True, "user_details": {"userName": user["name"], "mobile": user["mobile"]}})
    else:
